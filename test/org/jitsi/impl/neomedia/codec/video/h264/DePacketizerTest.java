...
import static org.jitsi.impl.neomedia.codec.video.h264.DePacketizer.kNalUSize;
import static org.jitsi.impl.neomedia.codec.video.h264.DePacketizer.kStapA;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertFalse;
import static org.junit.Assert.assertTrue;

public class DePacketizerTest {
    ...
    private boolean buildAndVerifyStapA(int[] naluSizes, int offset, int length) {
        ...
        return dePacketizer.verifyStapANaluLengths(data, offset, length);
    }
    ...
    public void getUint16_BuildsUint16() {
        assertEquals(0x0, dePacketizer.getUint16(new byte[] {0x0, 0x0}, 0));
        assertEquals(0xffff, dePacketizer.getUint16(new byte[] {(byte) 0xff, (byte) 0xff}, 0));
        assertEquals(0x1234, dePacketizer.getUint16(new byte[] {0x12, 0x34}, 0));
    }

    @Test
    public void getUint16_ObeysOffset() {
        assertEquals(0x1234, dePacketizer.getUint16(new byte[] {(byte) 0xff, 0x12, 0x34}, 1));
    }

    @Test
    public void getUint16_MostSignificantBitSetInFirstByte() {
        assertEquals(0xff00, dePacketizer.getUint16(new byte[] {(byte) 0xff, 0}, 0));
    }

    @Test
    public void getUint16_MostSignificantBitSetInSecondByte() {
        assertEquals(0xff, dePacketizer.getUint16(new byte[] { 0, (byte) 0xff}, 0));
    }

    @Test
    public void verifyStapANaluLengths_EmptyData_ShouldPass() {
        assertTrue(dePacketizer.verifyStapANaluLengths(new byte[] { kStapA }, 1, 0));
    }

    @Test
    public void verifyStapANaluLengths_LengthLessThanOneNal_ShouldFail() {
        assertFalse(dePacketizer.verifyStapANaluLengths(new byte[] { kStapA }, 1, 1));
        assertFalse(dePacketizer.verifyStapANaluLengths(new byte[] { kStapA }, 1, 2));
    }

    @Test
    public void verifyStapANaluLengths_PacketTooShort_ShouldFail() {
        assertFalse(dePacketizer.verifyStapANaluLengths(new byte[] { kStapA, 0, 1 }, 1, 3));
    }

    @Test
    public void verifyStapANaluLengths_ValidPacketWithOneNal_ShouldPass() {
        assertTrue(dePacketizer.verifyStapANaluLengths(new byte[] { kStapA, 0, 1, (byte)0xfa }, 1, 3));
    }
